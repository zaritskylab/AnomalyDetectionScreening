...
class ProfilingAnomalyDetector:
    ...
    def save_anomalies(self, input_profiles, normalize_reps=True,save_path = None):
        ...
        test_ctrl = input_profiles[input_profiles['Metadata_set'] == 'test_ctrl'][self.features].values
        test_treat = input_profiles[input_profiles['Metadata_set'] == 'test_treat'][self.features].values 

        diffs_ctrl = self.preds['test_ctrl'] - test_ctrl
        diffs_treat = self.preds['test_treat'] -  test_treat

        if normalize_reps:
            test_ctrl.loc[:, self.features] = diffs_ctrl
            test_treat.loc[:, self.features] = diffs_treat
            test_out = pd.concat([test_ctrl, test_treat], axis=0)
            test_out_normalized = normalize(test_out, self.features, self.modality, normalize_condition='test_ctrl', plate_normalized=0, norm_method="standardize")
        else:
            test_ctrl.loc[:, self.features] = diffs_ctrl
            test_treat.loc[:, self.features] = diffs_treat
            test_out = pd.concat([test_ctrl, test_treat], axis=0)
        ...
        if not self.debug_mode and save_path is not None:
            save_profiles(test_out_normalized, save_path)
